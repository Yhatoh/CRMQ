...
class Rank {
  public:
	 ...
	 */
	virtual uint64_t rank(std::size_t pos) const = 0;
	 ...
	 */
	uint64_t rank(std::size_t from, std::size_t to) const { return rank(to) - rank(from); }
	 ...
	 */
	virtual uint64_t rankZero(std::size_t pos) const { return pos - rank(pos); }
	 ...
	 */
	uint64_t rankZero(std::size_t from, std::size_t to) const { return rankZero(to) - rankZero(from); }
